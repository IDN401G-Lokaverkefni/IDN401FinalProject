...
\texttt{hasExam}: Hér er ítrað yfir $ C $ og lagt saman yfir $ E $ þar sem $ e $ er stærð mengis $ E $:

% Er í alvörunni verið að summa yfir stökin í E, setti þess vegna efri og neðri mörk á seinni summuna
$$\sum_{j \in C} \left( \sum_{i = 1}^{e} x_{ij} = 1 \right)$$

\texttt{maxInSlots}: Ítrað er yfir $ E $ og lagt saman yfir $ C $ þar sem $ c $ er stærð mengis $ C $ og $ j $ vísar á j-ta stak $ C $ og aftur lýsir stuðull $b_{j}$ sem táknar fjöldi nemenda $ b $ í námskeiði $ j $:

$$\sum_{i \in E} \left( \sum_{j = 1}^{c} x_{ij} \cdot b_{j} <= 450 \right)$$
...
\texttt{noExams}: Ítrum aftur yfir $ E $ og $ C $ og notum einnig stærð $ g_{} $
